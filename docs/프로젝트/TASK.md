...
|---|------|------|------|-----------|------------|
| 4-5 | 아티클 | 팔란티어 심층분석 시리즈 (6편) | 사용자 + AI | 도입편 published 완료. 0편(기업조망)/1편(Ontology)/2편(고객·경쟁) v2+x_publish+시각화 완료 (4/3) | **[사용자] 도입편·0편·1편·2편 순차 X 게시** → 3편 퇴고 |
...
| 1-32 | 시스템 | X 시황 포스트 자동 생성 + 텔레그램 전송 | AI + 사용자 | `generate_x_post.py` 완성. US/KR/GEO 3카테고리 생성+전송 동작 확인 (4/4) | **[사용자] 텔레그램 포스트 품질 피드백** → 프롬프트 튜닝, cron 자동화 검토 |
| 1-33 | 시스템 | cron 뉴스 수집 URL/도메인 노이즈 제거 | AI | 3단 필터 적용 완료 (A+C+D). 92개 오염 엔티티 삭제 (4/6) | ✅ 완료 |
| 1-34 | 시스템 | 심층분석 파이프라인 + cron 재설계 | AI | deep_analysis.py 완성. cron: 수집 15분→1시간, 심층분석 하루2회(05:30/17:30 KST) Ollama. 문제시 Claude API 전환 (4/6) | 퀄리티 모니터링 후 브리핑 연동 |
| 1-35 | 시스템 | InvestOS 이벤트 체계 대개편 | AI | EventType 분리 + 스토리 체이닝 + 타임라인 뷰 완료 (4/7 야간) | 백필 퀄리티 확인, 이벤트 중복 체크 로직 추가 |
| 1-36 | 시스템 | 관계도 복잡성 해소 — Top10 필터 + 설명 UX 개선 | AI | Top N 필터 + 카테고리 확장 완료 (4/7 야간) | 관계선 클릭 사이드패널 (다음 단계) |
...
| 1-30 | 시스템 | GeoInvest — 세상 이슈 관계도 통합 플랫폼 | P1 | 사용자 + AI | 2026-03-31 | 진행 | 디자인+엔지니어링 리뷰 완료. 아래 상세 참조 |
...
| 5-9 | 리서치 | 바이오 섹터 리서치 허브 — 4축+AI+인프라 체계 | P2 | 사용자 + AI | 2026-04-05 | 진행 | `data/research/sectors/bio/` 구조 생성. 축별 기술·기업 리서치 축적 → 아티클 발전 |
